...
import { articleListCmd, articleCreateCmd, articleGetCmd, articleUpdateCmd, articleDeleteCmd, } from './commands/article.js';
import { articleCreateFromYamlCmd } from './commands/articleFromYaml.js';
import { postListCmd, postCreateCmd, postGetCmd, postUpdateCmd, postDeleteCmd, postCreateFromTextCmd, postCreateFromFileCmd, } from './commands/post.js';
import { commentListCmd, commentListMineCmd, commentCreateCmd, commentDeleteCmd, commentLikeCmd, } from './commands/comment.js';
import { notificationListCmd, notificationUnreadCmd, } from './commands/notification.js';
...
article
    ...
    await articleCreateCmd(options.title, options.summary, content, options.status, getFormat(program));
});
article
    .command('get')
...
post
    ...
    await postCreateCmd(options.title, content, options.status, getFormat(program));
});
post
    .command('get')
...
post
    ...
    .requiredOption('--status <status>', 'draft or published')
    .action(async (options) => {
    await postCreateFromTextCmd(options.title, options.content, options.status, getFormat(program));
});
post
    .command('create-file')
    ...
    .requiredOption('--status <status>', 'draft or published')
    .action(async (options) => {
    await postCreateFromFileCmd(options.title, options.file, options.status, getFormat(program));
});
...
comment
    ...
    await commentCreateCmd(options.targetType, options.targetId, options.rootType, options.rootId, options.content, options.replyToUserId, anchor, getFormat(program));
});
comment
    .command('delete')
